...
const Cart = ({ activeCart, setActiveCart, shoes }) => {
  ...
  const handleClickPages = () => {
    ...
    history(`/making-order`);
  };

  return (
    <div className="cart__user">
      <svg
        className={activeCart ? "btn__svg active" : "btn__svg"}
        ...
        <div className="header__menu">
          ...
          {items.length > 0 ? (
            <div className="cart-menu__arrange">
              ...
              <button onClick={handleClickPages} className="btn">
                Go to the design
              </button>
            </div>
          ) : null}
      ...
      <div className="items__cart">{items.length}</div>
    </div>
  );
};
